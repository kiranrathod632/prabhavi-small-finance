...
// import { processOverduePenalties } from '../services/penaltyService.js';
...
export const startSmsCronJobs = () => {
  ...
  // --------------------------------------------------------------
  // NEW: Daily penalty update for overdue EMIs (runs at 12:05 AM)
  // --------------------------------------------------------------
  cron.schedule(
    '5 0 * * *',
    async () => {
      try {
        console.log('[Cron] Daily penalty update started (12:05 AM)');
        const result = await applyOverduePenalties();
        console.log(
          `[Cron] Penalty update completed: ${result.updated} of ${result.processed} overdue EMIs updated.`
        );
      } catch (e) {
        console.error('[Cron] Daily penalty update error:', e.message);
      }
    },
    { timezone: 'Asia/Kolkata' }
  );
  console.log('[Cron] Daily penalty update ENABLED — runs at 12:05 AM IST');
