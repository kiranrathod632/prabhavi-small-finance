...
const AdminLoans = () => {
  ...
  const [loadingEmis, setLoadingEmis] = useState({});
  ...
  const fetchLoanEmis = async (loanId) => {
    ...
    try {
      ...
      const emis = Array.isArray(res.data?.data) ? res.data.data : [];
      setEmisData(prev => ({ ...prev, [loanId]: emis }));
    } catch (error) {
      toast.error(getErrorMessage(error));
    } finally {
      setLoadingEmis(prev => ({ ...prev, [loanId]: false }));
    }
  };

  // NEW: Toggle loan expansion
  const toggleLoanExpand = (loanId) => {
    const isExpanded = !expandedLoans[loanId];
  ...
  // NEW: Calculate EMI stats for a loan
  const getEmiStats = (loanId) => {
    const emis = emisData[loanId] || [];
    const pendingEmis = emis.filter(e => e.status === 'pending' || e.status === 'overdue').length;
    const totalEmis = emis.length;
    const pendingAmount = emis
      .filter(e => e.status === 'pending' || e.status === 'overdue')
      .reduce((sum, e) => sum + (e.amount || 0) + (e.penalty || 0), 0);
    return { pendingEmis, totalEmis, pendingAmount };
  };
  ...
  return (
    <div className="page-stack">
      ...
      {/* Clean table list with expandable EMIs */}
      <div className="card overflow-hidden">
        <div className="data-table-wrap">
          <table className="data-table">
            ...
            <tbody>
              {loans.map((loan, index) => {
                const loanEmis = emisData[loan._id] || [];
                const { pendingEmis, totalEmis, pendingAmount } = getEmiStats(loan._id);
                const isExpanded = expandedLoans[loan._id];
                  ...
                  <>
                    <tr key={loan._id}>
                      ...
                      <td className="text-center !whitespace-nowrap">
                        <div className="table-actions flex !flex-row !flex-nowrap items-center gap-1 justify-center whitespace-nowrap">
                          {loanActions(loan)}
                          {/* NEW: Eye button for viewing EMIs */}
                          <button
                            type="button"
                            ...
                            title={isExpanded ? 'Hide EMIs' : 'View EMIs'}
                            disabled={loadingEmis[loan._id]}
                          >
                            {loadingEmis[loan._id] ? (
                          ...
                          <div className="bg-slate-50/50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-700">
                            {loadingEmis[loan._id] ? (
                              <div className="p-4 text-center">
                                ...
                                <span className="ml-2 text-sm text-slate-500">Loading EMIs...</span>
                              </div>
                            ) : loanEmis.length > 0 ? (
                              <>
                                  ...
                                  <table className="w-full text-sm">
                                    <thead className="bg-transparent">
                                      <tr className="border-b dark:border-gray-700 text-left">
                                        ...
                                        <th className="p-2 text-right text-xs font-medium text-slate-500">Principal</th>
                                        <th className="p-2 text-right text-xs font-medium text-slate-500">Interest</th>
                                    ...
                                    <tbody>
                                      {loanEmis.map((emi) => (
                                        <tr key={emi._id} className="border-b dark:border-gray-700/50 hover:bg-white dark:hover:bg-slate-700/50 transition-colors bg-white dark:bg-slate-800">
                                          ...
                                          <td className="p-2 text-right text-xs text-red-500">{formatCurrency(emi.penalty)}</td>
                                          <td className="p-2 text-xs whitespace-nowrap">{formatDate(emi.dueDate)}</td>
                                          <td className="p-2 text-center"><Badge status={emi.status} /></td>
                                          <td className="p-2 text-xs whitespace-nowrap">{formatDate(emi.paidDate) || 'N/A'}</td>
                                        </tr>
                                      ))}
                                    </tbody>
                                  </table>
      ...
      >
        <div className="space-y-6 max-h-[70vh] overflow-y-auto">
          ...
          <div>
            <h3 className="text-lg font-semibold mb-3">{t('adminLoans.selectTenure')} <span className="text-red-500">*</span></h3>
            <div className="flex flex-wrap gap-2">
              {/* TENURE_OPTIONS commented out as per original */}
            </div>
            <div className="mt-3">
              <label className="label">{t('adminLoans.manualTenure')}</label>
              <input
