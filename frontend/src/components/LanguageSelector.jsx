...
const LanguageSelector = ({ className = '', onChange, dark = false }) => {
  ...
  return (
    <label className={`inline-flex items-center gap-1 sm:gap-1.5 ${className}`}>
      <span
        className={`text-[10px] sm:text-xs font-bold tracking-wide ${
          dark ? 'text-white/80' : 'text-primary-600 dark:text-primary-300'
        }`}
      >
      </span>
      <select
